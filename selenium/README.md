...
### 2.1 极速双模登录 (Requests + Selenium)
为了在效率与成功率之间取得平衡，爬虫实现了两阶段登录模式：
1. **Cookie 缓存复用**：登录成功后的 Cookie 会被加密/持久化存入用户目录下的 `~/.hut_session.json`。后续运行会优先校验缓存，若未过期（TTL 4小时）则直接复用，**跳过所有登录步骤（耗时约 0.5s）**。
2. **Requests 直连登录 (P0 推荐)**：如果缓存失效，爬虫首先通过 Requests 获取教务系统的 RSA 公钥，在内存中完成密码加密并模拟登录。**无需打开浏览器，全程无感同步（耗时约 2s）**。
3. **Selenium 兜底登录**：若直连登录失败（如需图形验证码或滑块），则自动唤起 Chrome 浏览器让用户进行登录，成功后截获 Cookie 并更新缓存。

### 2.2 智能动态分页 (Dynamic Pagination)
由于成绩与考试安排可能存在多页数据，爬虫摒弃了硬编码 `pageSize` 的做法，而是会**先请求第 1 页，解析响应 JSON 中的 `count`（总条数）与当前页数据长度，动态计算总页数，并自动循环拉取后续页面**进行合并。
...
确保本机已安装 **Python 3.10+** 及 **Chrome 浏览器**。

```bash
pip install -r requirements.txt
```

> **主要依赖**：`requests` (网络请求)、`beautifulsoup4` (HTML解析)、`pycryptodome` (密码RSA加密)、`selenium` (浏览器兜底登录)、`webdriver-manager` (浏览器驱动自动管理)。
...
| `--headless`| `False` | 命令行加此参数即可 | 兜底启动 Selenium 时是否采用无头模式 |

---

## 5. 数据输出规范

详见项目根目录文档下的 [对接开发指南](../docs/crawler_integration_guide.md) 以获取完整的 JSON 数据结构规范。
